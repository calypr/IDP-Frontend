...
const DataComponent = ({ data }: { data: EdgeData }) => {
  ...
  const groupedItems: { [key: string]: { [key: string]: boolean } } =
    mergedItems.reduce((acc: { [key: string]: any }, item) => {
      if (!acc[item.id]) {
        acc[item.id] = {};
      }
  ...
  const finalItems: {
    ...
    hasSpecimen: boolean;
    hasDocument: boolean;
  }[] = Object.entries(groupedItems).map(([id, types]) => ({
    id,
  ...
  return (
    ...
    >
      {finalItems.map((item) => (
        ...
        >
          <Paper
            {...{ children: 'Content', sx: { padding: '10px', shadow: 'xs' } }}
          >
            {' '}
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {item.hasEncounter && (
                <Text style={{ margin: 10 }}>✅ Encounter</Text>
              )}
              {item.hasSpecimen && (
                <Text style={{ margin: 10 }}>✅ Specimen</Text>
              )}
              {item.hasDocument && (
                <Text style={{ margin: 10 }}>✅ DocumentReference</Text>
              )}
              <div>
                <div>{item.id}</div>
              </div>
...
const SamplePage = ({ headerProps, footerProps, pageProblems }: NavPageLayoutProps) => {
  ...
  const fetchEdgeData = async (
    ...
    return gripApiFetch(
      ...
      (result: gripApiResponse<EdgeData | unknown | undefined>) =>
        result.data as EdgeData,
    );
  };
  const pageSize = 50;
  ...
  const fetchData = async (limit: number, offset: number) => {
    ...
    try {
      const [docResult, encounterResult, specimenResult] = await Promise.all([
        fetchEdgeData(
          `query PatientIdsWithDocumentEdge($limit: Int, $offset: Int) {
        ...
        fetchEdgeData(
          `query PatientIdsWithEncounterEdge($limit: Int, $offset: Int) {
        ...
        fetchEdgeData(
          `query PatientIdsWithSpecimenEdge($limit: Int, $offset: Int) {
