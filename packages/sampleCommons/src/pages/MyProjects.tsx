...
import { getNavPageLayoutPropsFromConfig, NavPageLayout, NavPageLayoutProps, ProtectedContent } from '@gen3/frontend';
...
interface TreeViewProps { demoData: Program[] }
...
const convertAuthZToPrograms = (authz: any) : Program[] => {
  ...
  const paths = Object.keys(authz).filter(
    (resource) => {
      const path = resource.split('/');
      return path?.length === 5 && path[1] === 'programs'
      && path[3] === 'projects';
    }
  );
...
export const ProgramProjectsDisplay = ({ headerProps, footerProps }: NavPageLayoutProps) => {
      ...
      <ProtectedContent>
        <div className="w-full relative">
          <Container size="lg" px="md">
            <Title size="h1" className="py-2 px-4">List of Projects</Title>
            {isAuthZLoading &&
