...
const runtime: ExplorerRuntimeV1 = {
  ...
  outputs: [{
    ...
    title: 'People',
    rowLabel: 'person',
    selector: { recipe: 'recipe-1', translationVersion: 'r1', output: 'Patient' },
    columns: {
...
describe('ExplorerRuntimeV1 viewer projection', () => {
  it('uses server emissions, labels, selectors, filters, and charts directly', () => {
    ...
    expect(panel.guppyConfig.loomDataset).toEqual({ recipe: 'recipe-1', translationVersion: 'r1', output: 'Patient' });
    ...
    expect(panel.charts).toEqual({ patient_status: { chartType: 'pie', title: 'State' } });
