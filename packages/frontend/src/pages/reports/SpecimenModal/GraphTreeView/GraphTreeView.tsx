...
const GraphTreeView = ({
  ...
  const d3Edges = edges.map((edge) => ({
    parentId: specimenIdToLabel[edge[0]],
    id: specimenIdToLabel[edge[1]],
  })) as Record<string, string>[];

  // create root, providing a data.label for each node
  const root = stratify()(d3Edges).sort(
  ...
  const depthCounts = d3Tree.descendants().reduce(
    (acc, node) => {
  ...
  const focusNode = d3Tree
    .descendants()
    .find((node) => node.data.id === focusLabel);
  ...
  const reactFlowEdges = d3Edges.map((edge) => ({
    ...
    data: {
      betweenAncestors:
        focusAncestors.has(edge.parentId) && focusAncestors.has(edge.id),
    },
