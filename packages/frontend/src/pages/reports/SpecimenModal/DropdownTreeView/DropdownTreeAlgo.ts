...
export const edgesToNestedTree = (edges: Array<Array<string>>) => {
  ...
  // FIX: Convert the Set to an Array before iterating
  for (const root of Array.from(rootNodes)) {
...
const findRootNodes = (tree: Record<string, any>) => {
  ...
  const childNodes = new Set(Array.from(Object.values(tree)).flat());

  return Array.from(allNodes).filter((node) => !childNodes.has(node));
};
