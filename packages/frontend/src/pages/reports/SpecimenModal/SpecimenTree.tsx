...
const SpecimenTree = ({
    ...
    variables: {
      filter: {
        AND: [
          ...
          {
            EQ: {
              specimen_sample_family_id: `${sampleFamilyId}`,
            },
          },
        ],
      },
  ...
  const specimenDicts = extractQueryContent(
    specimenFamilyData,
    'specimen',
    '',
  ) as QueryContent;

  const specimenIdToLabel = specimenDicts.reduce(
    (acc, specimen: ResourceDict) => {
      acc[specimen.specimen_id] =
        `${specimen.specimen_identifier} (${specimen.specimen_sample_type})`;
      return acc;
    },
    {},
  ...
  const edges = specimenDicts.map((specimen: ResourceDict) => [
    specimen.specimen_parent,
